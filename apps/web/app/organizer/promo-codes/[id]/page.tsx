...
export default function EditPromoCodePage() {
  ...
  const fetchPromoCode = async () => {
    try {
      setLoading(true);
      const response = await organizerPromoCodeApi.get(promoCodeId);
      if (response.data.success) {
        const data = response.data.data;
        setCode(data.code);
        setDescription(data.description || '');
        setDiscountType(data.discountType);
        setDiscountValue(Number(data.discountValue));
        setMaxUses(data.maxUses?.toString() || '');
        setMaxUsesPerUser(data.maxUsesPerUser);
        setCurrentUses(data.currentUses);
        ...
        setValidFrom(formatDateForInput(data.validFrom));
        setValidUntil(formatDateForInput(data.validUntil));
      } else {
        setError('Failed to load promo code');
      }
    } catch (err: any) {
      console.error('Error fetching promo code:', err);
      setError(err.response?.data?.error || 'Failed to load promo code');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await organizerPromoCodeApi.update(promoCodeId, {
      ...
      if (response.data.success) {
        ...
        setError(response.data.error || 'Failed to update promo code');
      }
    } catch (err: any) {
      console.error('Error updating promo code:', err);
      setError(err.response?.data?.error || 'Failed to update promo code');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      ...
      <h1 className="text-3xl font-bold mb-8">Edit Promo Code</h1>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        <p className="text-sm text-blue-800">
          <strong>Current Usage:</strong> {currentUses}
