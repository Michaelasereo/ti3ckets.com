...
export default function EditEventPage() {
  ...
  const fetchEvent = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setVenueAddress(eventData.venueAddress);
        setCity(eventData.city);
        setCountry(eventData.country);
        setIsVirtual(eventData.isVirtual);
        setIsSeated(eventData.isSeated);
        ...
        setSaleStart(formatDateForInput(eventData.saleStart));
        setSaleEnd(formatDateForInput(eventData.saleEnd));
        
        // Set ticket types with IDs
        setTicketTypes(
  ...
  const validateStep = (stepNum: number): boolean => {
    ...
    if (stepNum === 2) {
      if (!venueName.trim() && !isVirtual) {
        setError('Venue name is required');
        return false;
      }
      if (!venueAddress.trim() && !isVirtual) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await organizerApi.updateEvent(eventId, {
        title,
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-4 py-8">
        <OrganizerEventNav eventId={eventId} />
        <LoadingSkeleton variant="lines" count={8} className="max-w-4xl" />
      </div>
    );
  }
  ...
  if (!event) {
    return (
      <div className="container mx-auto px-4 py-8">
        <p>Event not found</p>
        <Link href="/organizer/dashboard" className="text-primary-800 hover:underline">
          Back to Dashboard
        </Link>
      </div>
    );
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <OrganizerEventNav eventId={eventId} eventTitle={event?.title} showSeats={!!event?.isSeated} />
      ...
      <h1 className="text-3xl font-bold text-primary-900 mb-8">Edit Event</h1>
            ...
            <div key={index} className="border border-gray-200 rounded-xl p-4 space-y-4">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <h3 className="font-semibold">
                    Ticket Type {index + 1} {ticketType.id && '(Existing)'}
                  </h3>
                  {ticketType.price === 0 ? (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">Set to 0 for free tickets</p>
                </div>
                <div>
